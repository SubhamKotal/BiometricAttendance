...
public class TrainHelper {
    ...
    public static final String TRAIN_FOLDER = "Training";
    public static String TRAIN_FOLDER_PATH = "";
    public static int PHOTOS_TRAIN_QTY = -1;
...
//    public static final String FILE_NAME_PATTERN = "%d_%d.jpg";
//    public static final int PHOTOS_TRAIN_QTY=0;


    public static MatVector photos;
    public static Mat labels;


    // Count number of files in any given directory for any given extension
    public static int getNumberOfFiles(String TRAIN_FOLDER_PATH , String ext){

        File file = new File(TRAIN_FOLDER_PATH);
    ...
    public static void setTrainFolderPath(Context context){
        String dir_path = Environment.getExternalStorageDirectory()
                + "/Android/data/"
                + context.getPackageName()
                + "/"
                + TRAIN_FOLDER;
        TRAIN_FOLDER_PATH = dir_path;
    }

    public static void setPhotosTrainQty(Context context){
        setTrainFolderPath(context);
        int qty = getNumberOfFiles(TRAIN_FOLDER_PATH, ".jpg");
        PHOTOS_TRAIN_QTY = qty;
        Toast.makeText(context, "PHOTOS_TRAIN_QTY is now set.", Toast.LENGTH_SHORT).show();
    }

//    public static final int PHOTOS_TRAIN_QTY = getNumberOfFiles(TRAIN_FOLDER_PATH, ".jpg");

    //Deletes all photos and clasifiers in the train directories to reset everything
    public static void reset(Context context) throws Exception {
        File photosFolder = new File(TRAIN_FOLDER_PATH);
    ...
    // Checks if a trained model is already present
    ...
    // Returns Number of photos in the training directory (.jpg , .gif , .png)
    ...
    //trains the classsifier and saves it.
    public static boolean train(Context context) throws Exception {
        Toast.makeText(context, "Starting to train the Recognition model", Toast.LENGTH_SHORT).show();

        File photosFolder = new File(TRAIN_FOLDER_PATH);
        if (!photosFolder.exists()) {
            Toast.makeText(context, "Photos Folder not found!!", Toast.LENGTH_SHORT).show();
            Toast.makeText(context, TRAIN_FOLDER_PATH, Toast.LENGTH_SHORT).show();
            return false;
        };
        ...
        File[] files = photosFolder.listFiles(imageFilter);
        photos = new MatVector(files.length);
        labels = new Mat(files.length, 1, CV_32SC1);
        IntBuffer rotulosBuffer = labels.createBuffer();
        int counter = 0;

        Toast.makeText(context, "Starting to build photos and labels", Toast.LENGTH_SHORT).show();
        for (File image : files) {
            Mat photo = imread(image.getAbsolutePath(), IMREAD_GRAYSCALE);
            int classe = Integer.parseInt(image.getName().split("_")[0]);
...
//        FaceRecognizer lbph = createLBPHFaceRecognizer(2,9,9,9,1);
        ...
        Toast.makeText(context, "Creating the FisherFaces Recognition model", Toast.LENGTH_SHORT).show();
...
//        File f = new File(photosFolder, EIGEN_FACES_CLASSIFIER);
        File f = new File(photosFolder, FISHER_FACES_CLASSIFIER);
        f.createNewFile();
        Toast.makeText(context, "Saving the FisherFaces Recognition model", Toast.LENGTH_SHORT).show();
